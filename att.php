...
mysql_select_db($dbname) or die('database selection problem');
  $ida=$_POST['roll'];
  $results=mysql_query("SELECT * FROM `login` WHERE `universityroll` = '$ida' AND hostel_id='$h'");
  $res=mysql_fetch_assoc($results);
  $name1=$res['firstname']." ". $res['lastname'];
  $id=$res['id'];

           echo "<font size=5><CENTER>Attendence of <B>$name1</b><CENTER><BR></font>";
     ...
     while(strcmp($from,$to)<=0 && strcmp($from,date("Y-m-d"))<=0)
       {
            $sql="SELECT * FROM attendence WHERE `date`='$from' AND `student_id`='$id' AND hostel_id='$h'";
